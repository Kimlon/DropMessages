...
public protocol DropMessagesStatable {
...
public enum DropMessagesState: DropMessagesStatable {
...
public final class DropMessages: UIView {
    ...
    fileprivate var duration: TimeInterval = DropMessages.PRESET_DURATION
    ...
    @objc func upFromTimer(_ timer: Timer) {
        if let interval = timer.userInfo as? Double {
            DropMessages.up(self, interval: interval)
        }
    }
...
extension DropMessages {
    public class func down(_ status: String, state: DropMessagesState = .default, duration: Double = 4.0, action: DropAction? = nil) {
        show(status, state: state, duration: duration, action: action)
    }

    public class func down<T: DropMessagesStatable>(_ status: String, state: T, duration: Double = 4.0, action: DropAction? = nil) {
        show(status, state: state, duration: duration, action: action)
    }

    fileprivate class func show(_ status: String, state: DropMessagesStatable, duration: Double, action: DropAction?) {
        upAll()
        let drop = DropMessages(duration: duration)
    ...
    fileprivate class func up(_ drop: DropMessages, interval: TimeInterval) {
    ...
    public class func upAll() {
        ...
        for view in window.subviews {
            if let drop = view as? DropMessages {
                drop.up()
            }
        }
    }
...
extension DropMessages {
    fileprivate func setup(_ status: String, state: DropMessagesStatable) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(DropMessages.deviceOrientationDidChange(_:)), name: UIDevice.orientationDidChangeNotification, object: nil)
        
        layoutIfNeeded()
        addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(self.up(_:))))
        addGestureRecognizer(UIPanGestureRecognizer(target: self, action: #selector(self.pan(_:))))
    }
}
